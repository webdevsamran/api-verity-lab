...
CATALOG: dict[str, RuleSpec] = {spec.rule_id: spec for spec in [
    RuleSpec("BRK-OP-REMOVED", Severity.ERROR,
             "An operation was removed; existing callers will fail."),
    RuleSpec("BRK-RPC-REMOVED", Severity.ERROR,
             "A gRPC RPC was removed; existing callers will fail."),
    RuleSpec("BRK-OP-ADDED", Severity.INFO,
             "A new operation was added (additive, non-breaking)."),
    RuleSpec("BRK-RPC-ADDED", Severity.INFO,
             "A new gRPC RPC was added (additive, non-breaking)."),
    RuleSpec("BRK-PARAM-REMOVED", Severity.ERROR,
             "A request parameter was removed."),
    RuleSpec("BRK-PARAM-ADDED-REQUIRED", Severity.ERROR,
             "A new required request parameter was added."),
    RuleSpec("BRK-PARAM-ADDED-OPTIONAL", Severity.INFO,
             "A new optional request parameter was added."),
    RuleSpec("BRK-PARAM-REQUIRED", Severity.ERROR,
             "An optional request parameter became required."),
    RuleSpec("BRK-PARAM-OPTIONALIZED", Severity.INFO,
             "A required request parameter became optional."),
    RuleSpec("BRK-PARAM-TYPE-CHANGED", Severity.ERROR,
             "A request parameter's type/format changed."),
    RuleSpec("BRK-RESP-TYPE-CHANGED", Severity.WARN,
             "A response field's type changed; consumers may misparse values."),
    RuleSpec("BRK-CONSTRAINT-TIGHTENED", Severity.ERROR,
             "A request constraint was tightened; previously valid inputs fail."),
    RuleSpec("BRK-CONSTRAINT-LOOSENED", Severity.INFO,
             "A request constraint was loosened (previously invalid inputs pass)."),
    RuleSpec("BRK-RESP-CONSTRAINT-TIGHTENED", Severity.WARN,
             "A response constraint was tightened; returned values may fall "
             "outside what clients expect."),
    RuleSpec("BRK-ENUM-NARROWED-REQUEST", Severity.ERROR,
             "Request enum values were removed; clients sending old values fail."),
    RuleSpec("BRK-ENUM-NARROWED-RESPONSE", Severity.WARN,
             "Response enum values were removed; clients may encounter "
             "undeclared values at runtime."),
    RuleSpec("BRK-ENUM-WIDENED", Severity.INFO,
             "Enum values were added (additive)."),
    RuleSpec("BRK-REQ-FIELD-REMOVED", Severity.ERROR,
             "A request body field was removed."),
    RuleSpec("BRK-REQ-FIELD-ADDED-REQUIRED", Severity.ERROR,
             "A required field was added to a request body."),
    RuleSpec("BRK-REQ-FIELD-ADDED-OPTIONAL", Severity.INFO,
             "An optional field was added to a request body."),
    RuleSpec("BRK-REQ-FIELD-BECAME-REQUIRED", Severity.ERROR,
             "A request body field became required."),
    RuleSpec("BRK-RESP-FIELD-REMOVED", Severity.ERROR,
             "A response body field was removed; readers of it break."),
    RuleSpec("BRK-RESP-FIELD-ADDED", Severity.INFO,
             "A response body field was added (consumers ignore unknown fields)."),
    RuleSpec("BRK-REQ-BODY-REMOVED", Severity.ERROR,
             "The request body was removed."),
    RuleSpec("BRK-REQ-BODY-ADDED-REQUIRED", Severity.ERROR,
             "A required request body was added."),
    RuleSpec("BRK-REQ-BODY-ADDED-OPTIONAL", Severity.INFO,
             "An optional request body was added."),
    RuleSpec("BRK-REQ-BODY-REQUIRED", Severity.ERROR,
             "The request body became required."),
    RuleSpec("BRK-RESP-STATUS-REMOVED", Severity.ERROR,
             "A declared response status was removed."),
    RuleSpec("BRK-RESP-STATUS-ADDED", Severity.INFO,
             "A new response status was declared."),
    RuleSpec("BRK-HEADER-REMOVED", Severity.WARN,
             "A declared response header was removed."),
    RuleSpec("BRK-HEADER-ADDED", Severity.INFO,
             "A new response header was declared."),
    RuleSpec("BRK-SECURITY-CHANGED", Severity.ERROR,
             "Security requirements changed; unprepared clients fail auth."),
    RuleSpec("BRK-DEPRECATION-ADDED", Severity.WARN,
             "The operation is now deprecated; plan migration."),
    RuleSpec("BRK-DEPRECATION-REMOVED", Severity.INFO,
             "The deprecation marker was removed."),
    RuleSpec("BRK-MEDIA-TYPE-CHANGED", Severity.ERROR,
             "A request/response media type was added or removed."),
]}


def _constraint_change_is_tightening(attr: str, old: object, new: object) -> Optional[bool]:
    """Return True (tightened), False (loosened) or None (not comparable)."""
    if attr == "pattern":
        return None  # pattern changes are judged separately as WARN
    if attr in _TIGHTEN_ON_INCREASE:
        if isinstance(old, (int, float)) and isinstance(new, (int, float)):
            return new > old
    elif attr in _TIGHTEN_ON_DECREASE:
        if isinstance(old, (int, float)) and isinstance(new, (int, float)):
            return new < old
    return None
...
class BreakingEngine:
    ...
    def __init__(self, severity_overrides: Optional[dict[str, str]] = None) -> None:
        self.overrides = {
            rule_id: Severity(value)
            for rule_id, value in (severity_overrides or {}).items()
        }
    ...
    def _finding(self, rule_id: str, change: Change, message: str,
    ...
    def _evaluate_change(self, change: Change) -> list[Finding]:
        ...
        if kind == ChangeKind.PARAMETER_REQUIREDNESS:
            ...
            rule = (
                "BRK-PARAM-REQUIRED" if became_required else "BRK-PARAM-OPTIONALIZED"
            )
            return [self._finding(rule, change, change.description)]
        ...
        if kind == ChangeKind.ENUM_CHANGED:
            narrowed = "removed [" in change.description and "removed []" not in change.description
        ...
        if kind == ChangeKind.REQUEST_SCHEMA_CHANGED:
            ...
            if desc.endswith("added"):
                ...
                rule = (
                    "BRK-REQ-BODY-ADDED-REQUIRED" if required
                    else "BRK-REQ-BODY-ADDED-OPTIONAL"
                )
...
def evaluate_breaking(
    changes: list[Change], severity_overrides: Optional[dict[str, str]] = None
) -> list[Finding]:
