    ...
    if as_json:
        ...
        for key, value in data.items():
            if isinstance(value, list) and value and hasattr(value[0], "model_dump"):
                ...
                for item in value:
                    d = item.model_dump()
                    print(f"  [{d.get('severity', d.get('status', ''))}] "
...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    data = {
        "tool": "apiverity", "command": "validate", "spec": args.spec,
        "protocol": plugin.protocol().value, "title": service.title,
        "version": service.version, "operations": len(service.operations),
        "findings": all_findings, "errors": errors,
    }
    _emit(data, args.json)
    return EXIT_FINDINGS if errors else EXIT_OK


def _pair(args) -> tuple:
...
def cmd_diff(args: argparse.Namespace) -> int:
    ...
    _emit({"tool": "apiverity", "command": "diff",
           "old_version": old.version, "new_version": new.version,
           "changes": changes}, args.json)
    return EXIT_OK
...
def cmd_breaking(args: argparse.Namespace) -> int:
    ...
    if args.check_semver:
        policy = SemverPolicy(
            args.old_version or old.version, args.new_version or new.version,
    ...
    _emit({"tool": "apiverity", "command": "breaking",
           "old_version": old.version, "new_version": new.version,
           "changes": len(changes), "findings": findings, "errors": errors},
          args.json)
    return EXIT_FINDINGS if errors else EXIT_OK
...
def cmd_changelog(args: argparse.Namespace) -> int:
    ...
    text = generate_changelog(old.title, old.version, new.version, changes, findings,
...
def _start_mock(spec_path: str, port: int):
...
def cmd_test(args: argparse.Namespace) -> int:
    ...
    _emit({"tool": "apiverity", "command": "test", "base_url": args.base_url,
...
def cmd_workflow(args: argparse.Namespace) -> int:
    ...
    if not base_url:
        print("error: no base URL (pass --base-url or set base_url in manifest)",
              file=sys.stderr)
        return EXIT_USAGE
    try:
        ...
        _emit({"tool": "apiverity", "command": "workflow",
               "workflow": wf.name, "status": "error"}, args.json)
        return EXIT_UNREACHABLE
...
def cmd_mock(args: argparse.Namespace) -> int:
    ...
    service, _, _ = _load(args.spec)
    faults = FaultConfig(latency_ms=args.latency_ms, force_status=args.force_status,
                         malformed_json=args.malformed, rate_limit_after=args.rate_limit_after)
...
def cmd_coverage(args: argparse.Namespace) -> int:
    ...
    _emit({"tool": "apiverity", "command": "coverage",
           "overall_percent": report.overall_percent(), "report": report}, args.json)
    return EXIT_OK
...
def cmd_replay(args: argparse.Namespace) -> int:
    ...
    for e in entries_raw:
        ...
        entries.append(ReplayEntry(method=e["method"] or "GET", path=parsed.path or "/",
                                   query=e["query"], headers=e["request_headers"],
                                   body=e["request_body"]))
    try:
        report = replay_corpus(entries, args.base_url, allowed_hosts=args.allow_host,
...
def cmd_regression(args: argparse.Namespace) -> int:
    ...
    _emit({"tool": "apiverity", "command": "regression", "violations": violations,
           "report": report}, args.json)
...
def cmd_export(args: argparse.Namespace) -> int:
    ...
    _emit({"tool": "apiverity", "command": "export", "bundle": str(out),
           "files": sorted(checksums)}, args.json)
    return EXIT_OK
...
def cmd_rules(args: argparse.Namespace) -> int:
    ...
    rules = [{"rule_id": rid, "severity": spec.severity.value, "description": spec.description}
             for rid, spec in sorted(CATALOG.items())]
    _emit({"tool": "apiverity", "command": "rules", "count": len(rules), "rules": rules},
          args.json)
    return EXIT_OK
...
def cmd_self_test(args: argparse.Namespace) -> int:
    ...
    _emit({"tool": "apiverity", "command": "self-test", "ok": ok,
           "operations": len(service.operations), "spec_findings": len(findings)},
          args.json)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("validate"); p.add_argument("spec"); p.add_argument("--json", action="store_true"); p.set_defaults(func=cmd_validate)
    p = sub.add_parser("diff"); p.add_argument("old"); p.add_argument("new"); p.add_argument("--json", action="store_true"); p.set_defaults(func=cmd_diff)
    p = sub.add_parser("breaking"); p.add_argument("old"); p.add_argument("new")
    p.add_argument("--check-semver", action="store_true")
    p.add_argument("--old-version"); p.add_argument("--new-version")
    p.add_argument("--require-minor-for-warnings", action="store_true")
    p.add_argument("--severity-override", action="append")
    p.add_argument("--json", action="store_true"); p.set_defaults(func=cmd_breaking)
    p = sub.add_parser("changelog"); p.add_argument("old"); p.add_argument("new")
    p.add_argument("--html", action="store_true"); p.add_argument("--output"); p.set_defaults(func=cmd_changelog)
    p = sub.add_parser("test"); p.add_argument("spec"); p.add_argument("--base-url", required=True)
    p.add_argument("--seed", type=int, default=0); p.add_argument("--timeout", type=float, default=10.0)
    p.add_argument("--minimize", action="store_true"); p.add_argument("--json", action="store_true")
    p.set_defaults(func=cmd_test)
    p = sub.add_parser("workflow"); p.add_argument("manifest"); p.add_argument("--base-url")
    p.add_argument("--json", action="store_true"); p.set_defaults(func=cmd_workflow)
    p = sub.add_parser("mock"); p.add_argument("spec"); p.add_argument("--port", type=int, default=8090)
    p.add_argument("--latency-ms", type=int, default=0); p.add_argument("--force-status", type=int)
    p.add_argument("--malformed", action="store_true"); p.add_argument("--rate-limit-after", type=int)
    p.set_defaults(func=cmd_mock)
    p = sub.add_parser("coverage"); p.add_argument("spec")
    p.add_argument("--exercised", nargs="*"); p.add_argument("--json", action="store_true")
    p.set_defaults(func=cmd_coverage)
    p = sub.add_parser("drift"); p.add_argument("spec"); p.add_argument("--base-url", required=True)
    p.add_argument("--timeout", type=float, default=10.0); p.add_argument("--json", action="store_true")
    p.set_defaults(func=cmd_drift)
    p = sub.add_parser("replay"); p.add_argument("har"); p.add_argument("--base-url", required=True)
    p.add_argument("--allow-host", action="append", required=True)
    p.add_argument("--execute", action="store_true"); p.add_argument("--rate", type=float, default=10.0)
    p.add_argument("--i-know-this-is-production", action="store_true")
    p.add_argument("--json", action="store_true"); p.set_defaults(func=cmd_replay)
    p = sub.add_parser("baseline"); p.add_argument("spec"); p.add_argument("--base-url", required=True)
    p.add_argument("-o", "--output", default="perf-baseline.json")
    p.add_argument("--iterations", type=int, default=20); p.add_argument("--json", action="store_true")
    p.set_defaults(func=cmd_baseline)
    p = sub.add_parser("regression"); p.add_argument("spec"); p.add_argument("--base-url", required=True)
    p.add_argument("--baseline"); p.add_argument("--policy", action="append")
    ...
    p = sub.add_parser("plugins"); p.add_argument("--json", action="store_true"); p.set_defaults(func=cmd_plugins)
    p = sub.add_parser("rules"); p.add_argument("--json", action="store_true"); p.set_defaults(func=cmd_rules)
    p = sub.add_parser("self-test"); p.add_argument("--json", action="store_true"); p.set_defaults(func=cmd_self_test)
    return parser
