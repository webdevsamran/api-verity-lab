...
def _defined() -> dict[int, str]:
    src = (_ROOT / "apiverity/cli/commands/common.py").read_text(encoding="utf-8")
    return {
        int(m.group(2)): m.group(1).removeprefix("EXIT_")
        for m in re.finditer(r"^(EXIT_[A-Z_]+)\s*=\s*(\d+)", src, re.M)
    }
