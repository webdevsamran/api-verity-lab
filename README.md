...
One build, three scales. There is no enterprise edition, no paid tier and no
sponsor-only feature — a governance tool with a hidden half is a governance
tool you cannot audit.

| | What you would actually run |
|---|---|
| **One developer, one API** | `apiverity init` then `apiverity breaking old.yaml new.yaml`. The gate starts **off**, because a check that fails on its first run against an API with history gets removed rather than adopted. |
| **A team with several services** | `sweep` across the monorepo, `--consumers` for blast radius, ownership routing so a finding reaches the team that owns it, and rule packs so house style is a package rather than a fork. |
| **An organisation under audit** | Evidence packs for SOC 2 / ISO 42001 / DORA / the EU AI Act, a tamper-evident audit chain, OIDC, an air-gapped install, and a kill switch that is one command. |
| **A team shipping AI agents** | MCP tool-surface diffing, declared-vs-live drift, tool-description poisoning detection, a signed lockfile and call budgets. |

[Who this is for, in full →](docs/use-cases.md)
...
The catalog ships 79 rules across ERROR/WARN/INFO with per-rule severity
...
| [docs/faq.md](docs/faq.md) · [docs/use-cases.md](docs/use-cases.md) | The short answers, and which parts to switch on at your scale |
| [docs/roadmap-status.md](docs/roadmap-status.md) | All 134 planned items, each with evidence that is re-checked in CI |
| [SPONSORS.md](SPONSORS.md) · [ACKNOWLEDGEMENTS.md](ACKNOWLEDGEMENTS.md) | What sponsorship funds and does not buy; who this is built on and after |
...
**What is API contract governance?** Treating the contract as something checked
on every change rather than documentation updated afterwards: what changed, does
it break a consumer, does the running service still match, is it still fast
enough.

**How do I detect breaking changes in an OpenAPI spec?**
`apiverity breaking v1.yaml v2.yaml` — exit 1 when anything at or above your
threshold fires, and every finding carries a rule id you can look up with
`apiverity explain`.

**Is it only for OpenAPI?** No — OpenAPI 3.0/3.1/3.2, Swagger 2.0, AsyncAPI,
GraphQL, gRPC, MCP tool manifests and WSDL, through one contract model.

**Does it send my spec anywhere?** No. It makes an outbound request only when
you pass a URL, and you do not have to take that on trust: the
[egress map](docs/egress.md) is generated by walking the source for network
calls.

**Will it fail my build on day one?** Only if you ask it to. `init` writes the
gate off, `drift --baseline` fails on what is *newly* wrong, and suppressions
carry an owner, a reason and an expiry.

**Can I add rules without forking?** Yes — a rule pack is a normal Python
package declaring an entry point, or a YAML policy file if you would rather not
write Python.

**Can it run air-gapped?** Yes: container image, Helm chart, vendored schemas,
no telemetry.

[Twenty more answers →](docs/faq.md)

## Sponsorship

This project is Apache-2.0, runs on your machines, sends no telemetry and has
no paid tier. Sponsorship is what makes that sustainable rather than slowly
abandoned.

**[→ Sponsor on GitHub](https://github.com/sponsors/webdevsamran)** ·
[what it funds, and what it does not buy](SPONSORS.md)

It buys maintenance, version matrices, the quarterly competitor refresh and the
work of keeping every generated claim true. It does **not** buy priority
support, a feature on demand, a logo in any artifact this tool produces, or
influence over a finding — the benchmark publishes where this tool loses for
exactly that reason.

Free things that help as much: run it against a contract you own and
[open an issue about what it got wrong](https://github.com/webdevsamran/api-verity-lab/issues)
— a false positive is worth more here than a star.

## Credits

Built on other people's specifications and libraries, and after other people's
ideas. [ACKNOWLEDGEMENTS.md](ACKNOWLEDGEMENTS.md) names all of them: the
OpenAPI Initiative, AsyncAPI, the GraphQL Foundation, Google's protobuf and
gRPC, Anthropic's MCP, the W3C, OASIS, the IETF and OWASP; pydantic, httpx,
PyYAML, Flask, React, Pyodide and the rest; and the projects this learned from
— oasdiff, Schemathesis, Spectral, Pact, Dredd, Karate, Microcks, Buf, k6 and,
most of all, ESLint, which won on explanations.
